...
describe('Home', () => {
  ...
  test('renders the card with Google sign-in button', () => {
    ...
    const cardDescription = screen.getByText(/Are you from within New Era University?/i);
    expect(cardDescription).toBeInTheDocument();
    expect(cardDescription).toBeVisible();

    const googleSignInButton = screen.getByRole('button', { name: /Sign in with my institutional email/i });
  ...
  test('clicking on the "Sign in with my institutional email" button calls signInWithGoogle', async () => {
    render(<Home />);
    const signInButton = screen.getByRole('button', { name: /Sign in with my institutional email/i });
